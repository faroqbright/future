...
const Layout: React.FC<Props> = ({ children }) => {
  ...
  const handlePointerEnter = contextSafe((id: string) => {
    if (id === "uniqueId2") {
      gsap
        .timeline()
    ...
    } else if (id === "uniqueId1") {
      gsap
        .timeline()
        .to(".menuinsides", { opacity: 0, stagger: 0.1 })
        .to(".menuoutsides", { opacity: 1, stagger: 0.1 }, "<")
        .to(
          ".menuoutsides",
          {
            keyframes: { y: [0, 80, 0], opacity: [1, 0, 0] },
  ...
  const handlePointerLeave = contextSafe((id: string) => {
    if (id === "uniqueId2") {
